  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {moods.map((m) => {
        ...
        return (
          <button key={m.value ?? "any"} type="button" onClick={() => onChange(m.value)} className={cn("px-3 py-1.5 rounded-[8px] text-[12.5px] font-semibold transition-all duration-150 hover:scale-[1.04]", isActive ? "bg-accent-soft text-accent ring-1 ring-[color:var(--color-accent)]/60 hover:bg-accent/20 hover:ring-[color:var(--color-accent)]" : "bg-surface-2 text-muted hover:text-text hover:bg-surface")}>
            {m.label}
          </button>
        );
      })}
